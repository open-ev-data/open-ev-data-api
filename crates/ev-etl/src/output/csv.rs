...
pub fn generate(vehicles: &[Vehicle], output_path: &Path) -> Result<()> {
    ...
    for vehicle in vehicles {
        ...
        let sources: Vec<String> = vehicle
            .sources
            .iter()
            .map(|s| s.url.clone())
            .collect();
        ...
        writer.write_record([
            ...
            vehicle.variant.as_ref().map(|v| v.slug.clone()).unwrap_or_default(),
            vehicle.variant.as_ref().map(|v| v.name.clone()).unwrap_or_default(),
            format!("{:?}", vehicle.vehicle_type),
            format!("{:?}", vehicle.powertrain.drivetrain),
            vehicle.powertrain.system_power_kw.map(|v| v.to_string()).unwrap_or_default(),
            vehicle.powertrain.system_torque_nm.map(|v| v.to_string()).unwrap_or_default(),
            vehicle.battery.pack_capacity_kwh_gross.map(|v| v.to_string()).unwrap_or_default(),
            vehicle.battery.pack_capacity_kwh_net.map(|v| v.to_string()).unwrap_or_default(),
            vehicle.battery.chemistry.clone().unwrap_or_default(),
            vehicle.charging.dc.as_ref().map(|dc| dc.max_power_kw.to_string()).unwrap_or_default(),
            vehicle.charging.ac.as_ref().map(|ac| ac.max_power_kw.to_string()).unwrap_or_default(),
            vehicle.range.wltp_range_km().map(|v| v.to_string()).unwrap_or_default(),
            vehicle.range.epa_range_km().map(|v| v.to_string()).unwrap_or_default(),
