...
fn write_schema(file: &mut std::fs::File) -> Result<()> {
    ...
    writeln!(file, "-- Generated by ev-etl v{}", env!("CARGO_PKG_VERSION"))?;
    writeln!(file)?;
    writeln!(file, "BEGIN;")?;
    writeln!(file)?;
...
fn write_indexes(file: &mut std::fs::File) -> Result<()> {
    ...
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_vehicles_make ON vehicles(make_slug);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_vehicles_model ON vehicles(model_slug);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_vehicles_year ON vehicles(year);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_vehicles_composite ON vehicles(make_slug, model_slug, year, trim_slug);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_vehicles_type ON vehicles(vehicle_type);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_vehicles_json ON vehicles USING GIN (json_data);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_charge_ports_vehicle ON charge_ports(vehicle_id);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_range_ratings_vehicle ON range_ratings(vehicle_id);")?;
    writeln!(file, "CREATE INDEX IF NOT EXISTS idx_sources_vehicle ON sources(vehicle_id);")?;
