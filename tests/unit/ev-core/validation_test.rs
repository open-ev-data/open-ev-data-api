...
fn test_validate_slug_valid() {
    assert!(ev_core::validation::validate_slug("tesla").is_ok());
    assert!(ev_core::validation::validate_slug("model_3").is_ok());
    assert!(ev_core::validation::validate_slug("byd_dolphin_2024").is_ok());
}

#[test]
fn test_validate_slug_invalid() {
    assert!(ev_core::validation::validate_slug("").is_err());
    assert!(ev_core::validation::validate_slug("Tesla").is_err());
    assert!(ev_core::validation::validate_slug("model-3").is_err());
    assert!(ev_core::validation::validate_slug("model 3").is_err());
}

#[test]
fn test_validate_year_valid() {
    assert!(ev_core::validation::validate_year(2024).is_ok());
    assert!(ev_core::validation::validate_year(1900).is_ok());
    assert!(ev_core::validation::validate_year(2100).is_ok());
}

#[test]
fn test_validate_year_invalid() {
    assert!(ev_core::validation::validate_year(1899).is_err());
    assert!(ev_core::validation::validate_year(2101).is_err());
}

#[test]
fn test_validate_country_code() {
    assert!(ev_core::validation::validate_country_code("US").is_ok());
    assert!(ev_core::validation::validate_country_code("BR").is_ok());
    assert!(ev_core::validation::validate_country_code("usa").is_err());
    assert!(ev_core::validation::validate_country_code("U").is_err());
}

#[test]
fn test_validate_currency_code() {
    assert!(ev_core::validation::validate_currency_code("USD").is_ok());
    assert!(ev_core::validation::validate_currency_code("BRL").is_ok());
    assert!(ev_core::validation::validate_currency_code("usd").is_err());
    assert!(ev_core::validation::validate_currency_code("US").is_err());
}

#[test]
fn test_validate_url_valid() {
    assert!(ev_core::validation::validate_url("https://example.com").is_ok());
    assert!(ev_core::validation::validate_url("http://example.com").is_ok());
    assert!(ev_core::validation::validate_url("https://example.com/path").is_ok());
}

#[test]
fn test_validate_url_invalid() {
    assert!(ev_core::validation::validate_url("").is_err());
    assert!(ev_core::validation::validate_url("example.com").is_err());
    assert!(ev_core::validation::validate_url("ftp://example.com").is_err());
    assert!(ev_core::validation::validate_url("www.example.com").is_err());
}

#[test]
fn test_collect_errors_empty() {
    let items: Vec<String> = vec![];
    let result = ev_core::validation::collect_errors(items, |_: &String| Ok(()));
    assert!(result.is_ok());
}
...
fn test_collect_errors_all_valid() {
    ...
    let result = ev_core::validation::collect_errors(items, |s: &&str| {
        ev_core::validation::validate_slug(s)
    });
    assert!(result.is_ok());
}
